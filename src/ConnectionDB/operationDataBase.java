...
if(val2>val)
    ...
    // ta3 passenger count
    public static void searchData8(JTable table,String Column,int val)
    {
    ConnectionDB.MongoDB.Connect_MongoDB("AppMaster");
     DefaultTableModel model=(DefaultTableModel) table.getModel();
    DBCollection col=ConnectionDB.MongoDB.db.getCollection("taxi1");
...
if( min>val || val>max)
    ...
    Tools.showMessage("The number of erroeneous records are :"+table.getModel().getRowCount()+"Rows");
      ...
      public static void getObjectID(JTable table1,JTable table2)
     {
      ...
      public static void loadMatrix(JTable tab1,JTable tab2,JTable tab3,JTable tab4,JTable tab5,JTable tab6,JTable tab7,JTable tab8,JTable Matrix)
      {
         ...
         rules = new ArrayList<>(Arrays.asList(tab1, tab2, tab3, tab4, tab5, tab6, tab7,tab8));
         ...
         for (int i = 0; i<rules.size(); ++i){
             ...
             for(int k=0 ; k<currentJtable.getRowCount();++k){
                ...
                if(! matrix.containsKey(objId)){
                    matrix.put(objId, new ArrayList(Arrays.asList(0,0,0,0,0,0,0,0)));
                }
   ...
   double[] s2= new double[512];
      ...
      int z2;
      Hashtable<String, Integer> tt2 = new Hashtable<>();
        ...
        for(int i=0;i<7;++i){
            for(int j=i+1;j<8;++j){
                ...
                if(matrix.get(k2).get(i) == 1 && matrix.get(k2).get(j) == 1){
                    String tmp2 = Integer.toString(j) + "->" + Integer.toString(i);
                    if (!tt2.containsKey(tmp2)){
                        tt2.put(tmp2, 0);
                    }
                    tt2.put(tmp2, tt2.get(tmp2)+1);
                }
                z2++;
            }
        }
